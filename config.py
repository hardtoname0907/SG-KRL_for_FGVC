from utils.indices2coordinates import indices2coordinates
from utils.compute_window_nums import compute_window_nums
import numpy as np

CUDA_VISIBLE_DEVICES = '3'  # The current version only supports one GPU training


set = 'GEN'  # 3Different dataset with different
model_name = 'fface_1'

batch_size = 40  # 6——10
vis_num = batch_size  # The number of visualized images in tensorboard
eval_trainset = False  # Whether or not evaluate trainset
save_interval = 20
max_checkpoint_num = 100 #200
end_epoch = 100 #200
init_lr = 0.001
lr_milestones = [60, 100] # [50, 85]
lr_decay_rate = 0.1
weight_decay = 1e-4
stride = 32
channels = 2048
input_size = 448
gpu_ids=[0,1,2,3]
backboneres='resnet50'

# The pth path of pretrained model
if backboneres == 'resnet50':
    pretrain_path = './models/pretrained/resnet50-19c8e357.pth'
elif backboneres == 'resnet101':
    pretrain_path = './models/pretrained/resnet101-5d3b4d8f.pth'
else:
    raise ValueError('backbone resnet error')


if set == 'CUB':
    model_path = './checkpoint/cub'  # pth save path
    root = '/data/user/zhuyueran/models/MMAL/datasets/CUB_200_2011'  # dataset path './datasets/CUB_200_2011'
    num_classes = 200
    # windows info for CUB
    N_list = [2, 3, 2] # 每组窗口中需要选出的候选窗口数
    proposalN = sum(N_list)  # proposal window num，需要选出的总窗口数
    window_side = [128, 192, 256] # 窗口大小限制，看不出有啥用
    iou_threshs = [0.25, 0.25, 0.25] # 交并比阈值
    ratios = [[4, 4], [3, 5], [5, 3],
              [6, 6], [5, 7], [7, 5],
              [8, 8], [6, 10], [10, 6], [7, 9], [9, 7], [7, 10], [10, 7]]
    # ratios的三行就是小中大三组尺寸的窗口

elif set == 'DOG':
    model_path = './checkpoint/dog'  # pth save path
    root = '/data/user/zhuyueran/models/MMAL/datasets/Stanford_Dogs'  # dataset path './datasets/CUB_200_2011'
    num_classes = 120
    # windows info for CUB
    N_list = [2, 3, 2] # 每组窗口中需要选出的候选窗口数
    proposalN = sum(N_list)  # proposal window num，需要选出的总窗口数
    window_side = [128, 192, 256] # 窗口大小限制，看不出有啥用
    iou_threshs = [0.25, 0.25, 0.25] # 交并比阈值
    ratios = [[4, 4], [3, 5], [5, 3],
              [6, 6], [5, 7], [7, 5],
              [8, 8], [6, 10], [10, 6], [7, 9], [9, 7], [7, 10], [10, 7]]
    # ratios的三行就是小中大三组尺寸的窗口

elif set == 'NAB':
    model_path = './checkpoint/nab'  # pth save path
    root = '/data/user/zhuyueran/models/MMAL/datasets/NAbirds'  # dataset path '/data/user/zhuyueran/models/MMAL/datasets/NAbirds'
    num_classes = 555
    # windows info for CUB
    N_list = [2, 3, 2] # 每组窗口中需要选出的候选窗口数
    proposalN = sum(N_list)  # proposal window num，需要选出的总窗口数
    window_side = [128, 192, 256] # 窗口大小限制，看不出有啥用
    iou_threshs = [0.25, 0.25, 0.25] # 交并比阈值
    ratios = [[4, 4], [3, 5], [5, 3],
              [6, 6], [5, 7], [7, 5],
              [8, 8], [6, 10], [10, 6], [7, 9], [9, 7], [7, 10], [10, 7]]
    # ratios的三行就是小中大三组尺寸的窗口

else:
    # windows info for CAR and Aircraft
    N_list = [3, 2, 1]
    proposalN = sum(N_list)  # proposal window num
    window_side = [192, 256, 320]
    iou_threshs = [0.25, 0.25, 0.25]
    ratios = [[6, 6], [5, 7], [7, 5],
              [8, 8], [6, 10], [10, 6], [7, 9], [9, 7],
              [10, 10], [9, 11], [11, 9], [8, 12], [12, 8]]
    if set == 'CAR':
        model_path = './checkpoint/car'      # pth save path
        root = '/data/user/zhuyueran/models/MMAL/datasets/Stanford_Cars'  # dataset path
        num_classes = 196
    elif set == 'Aircraft':
        model_path = './checkpoint/aircraft'      # pth save path
        root = '/data/user/zhuyueran/models/MMAL/datasets/FGVC-aircraft'  # dataset path
        num_classes = 100
    elif set == 'LUX':
        model_path = './checkpoint/Luxury'      # pth save path
        root = '/data/user/zhuyueran/datasets/maotai/241112_latest_crop_fsp'  # dataset path
        num_classes = 8 # piqian&wujin8,yuanbiao6,maotai8
    elif set == 'GEN':
        model_path = './checkpoint/Geners'      # pth save path
        root = '/data/user/zhuyueran/datasets/fake_face_custom'  # dataset path
        num_classes = 2

'''indice2coordinates'''
window_nums = compute_window_nums(ratios, stride, input_size) 
# 计算448的图上步长32能产生多少窗口，其实就是14上步长1
indices_ndarrays = [np.arange(0,window_num).reshape(-1,1) for window_num in window_nums]
coordinates = [indices2coordinates(indices_ndarray, stride, input_size, ratios[i]) for i, indices_ndarray in enumerate(indices_ndarrays)] # 每个window在image上的坐标
coordinates_cat = np.concatenate(coordinates, 0)
window_milestones = [sum(window_nums[:i+1]) for i in range(len(window_nums))]
if set == 'CUB':
    window_nums_sum = [0, sum(window_nums[:3]), sum(window_nums[3:6]), sum(window_nums[6:])] 
    # 小中大三组窗口，每组窗口在总窗口列表中对应的索引
else:
    window_nums_sum = [0, sum(window_nums[:3]), sum(window_nums[3:8]), sum(window_nums[8:])]
